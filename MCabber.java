 ...
 */
// TODO while reading in, ask the username for each file 
...
public class MCabber extends Messenger {
    ...
    /**
     * A free selectable nickname for the user , e.g. the first name.
     */
    private String nickname;

    /**
     * A free selectable nickname for the chat partner , e.g. the first name.
     */
    ...
    // Input
    /**
     * Reads additional informations about the chat. MCabber doesn't provide any
     * details about the chatting people. So we have to ask the user for the
     * additional, essential infos (jid and the nicknames).
     */
        ...
        for(int i = 0; i < filenames.size(); i++) {
            try {
                ...
                while((line = buff.readLine()) != null) {
                    ...
                    // we only read send or received messages, no status
                    // messages
                    if((line.startsWith("MS") || line.startsWith("MR"))) {
                        ...
                        // received messages
                            ...
                            // this isn't quite beautiful, but we hope that the
                            // user didn't changed the standard filenames of
                            // mcabber...
                ...
                System.err.println("File " + filenames.get(i).getName()
                        + " not found!");
                System.exit(1);
            } catch(IOException e) {
                System.err
    ...
    public void writeOutputFile() {
        ...
        if(Cache.getInstance().getNumberAllMsg() != 0) {
            ...
            // we look for the name of the chat buddy and use it for the
            // filename
            for(int i = 0; i < Cache.getInstance().getNumberAllMsg(); i++) {
                if(Cache.getInstance().getMessage(i).getIsUser() == false) {
                    filename = new File(Cache.getInstance().getMessage(i)
                            .getBuddyID());
                    break;
                }
            }
            ...
            try {
                ...
                // if we didn't found a buddy nick, e.g. because there's only
                // one message of the user
                // we have to ask for the name.
                if(filename == null) {
                    do {
                        System.out
                                .print("The JID of your chat partner? [nick@jabber-server.tld] ");
                        BufferedReader br1 = new BufferedReader(
                                new InputStreamReader(System.in));
                        filename = new File(br1.readLine());
                    } while(!filename.getName().contains("@")
                            || !filename.getName().contains("."));
                }
                ;

                // if the filename already exists, we have to append the
                ...
                for(int i = 0; i < Cache.getInstance().getNumberAllMsg(); i++) {
                    if(Cache.getInstance().getMessage(i).getIsUser() == true)
                        ...
                        fw.write("MR ");
                    fw.append(Cache.getInstance().getMessage(i).getDate()
                            + "T"
                            + Cache.getInstance().getMessage(i).getTime()
                            + "Z 000 "
                            + Cache.getInstance().getMessage(i)
                                    .getMessageText());
