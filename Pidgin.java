...
public class Pidgin extends Messenger {
    ...
    /**
     * The user name of the user, who owns the logs. This is a free selectable
     * name, e.g. the first name.
     */
    ...
    private void readInputFileHTML(File filename) {
        ...
        try {
            ...
            // we create a temporary file where we convert the html file to the
            // txt logfile
            ...
            for(int a = 0; a < Cache.getInstance().getDiffDates(); a++) {
                ...
                for(int i = 0; i < Cache.getInstance().getNumberMsgDiffDates(); i++) {
                    if(Cache.getInstance().getMessage(i).getIsUser() == true)
                        ...
                        buddyid = Cache.getInstance().getMessage(i)
                                .getBuddyID();
                }
                ...
                try {
                    ...
                    DateFormat inputform = new SimpleDateFormat(
                            "yyyyMMdd HH:mm:ss");
                    Date d = inputform
                            .parse(Cache.getInstance().getMessage(0).getDate()
                                    + " "
                                    + Cache.getInstance().getMessage(0)
                                            .getTime());

                    // format date/time of sessionstart
                    DateFormat outputformText = new SimpleDateFormat(
                            "E d MMM yyyy HH:mm:ss z");
                    dateTimeText = outputformText.format(d);
                    DateFormat outputformFilename = new SimpleDateFormat(
                            "yyyy-MM-d.HHmmssZz");
                    dateTimeFilename = outputformFilename.format(d);
                } catch(ParseException pe) {
                    System.err
                            .println("Parse Exception. Please contact the author or report this as a bug.\n-------------details-------------");
                    pe.printStackTrace();
                    System.exit(1);
                }
                ;
                ...
                try {
                    ...
                    // the messaging protocol is missing, but i dont know how to
                    // handle this
                    fw.append("Conversation with " + buddyid + " at "
                            + dateTimeText + " on " + userid + "()");
                    fw.append(System.getProperty("line.separator"));
